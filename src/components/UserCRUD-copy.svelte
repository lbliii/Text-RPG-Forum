...
<form class="my-6 justify-center" on:submit|preventDefault={handleSubmit}>
	<div class="flex flex-col text-sm mb-2">
		...
		<Textarea
			type="text"
			bind:value={profile.likes}
			class="apperance-none shadow-sm border border-gray-200 p-2 m-2 focus:outline-none focus:border-gray-500 rounded-lg"
			id="likes"
			name="likes"
			rows="5"
			placeholder="Tell other writers what kind of plots and genres you like to write."
		/>
		<Label>Dislikes</Label>
		<Textarea
			type="text"
			bind:value={profile.dislikes}
			class="apperance-none shadow-sm border border-gray-200 p-2 m-2 focus:outline-none focus:border-gray-500 rounded-lg"
			id="dislikes"
			name="dislkes"
			rows="5"
			placeholder="Tell other writers what kind of plots and genres you do not like."
		/>
	</div>
