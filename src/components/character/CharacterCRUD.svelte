...
<Modal bind:open={openModal} size="xs" autoclose={false} title="{create ? 'Create ' : 'Edit'}  Character">
	...
	{#if $userStore.user_id}
		{#if !create || $charactersStore.length < 3 } 
	...
	{:else}
		<div class="flex flex-col justify-center items-center">
			<Heading tag="h2" class="mb-4">You must be logged in to create a character.</Heading>
			<P> Please log in or create an account to create a character.</P>
		</div>
	{/if}
</Modal>
